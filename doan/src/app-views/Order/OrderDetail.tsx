    ...
    <Container>
      ...
      <Content style={styles.container}>
            ...
            {STATUS_STEPS.map((status, index) => {
              ...
              return (
                <View key={status} style={styles.progressStep}>
                  <View style={[
                    styles.circle,
                    { backgroundColor: order?.order_status === 'cancelled' ? '#EF4444' : isActive ? '#3B82F6' : '#D1D5DB' }
                  ]}>
                    <Text style={styles.circleText}>{index + 1}</Text>
                  </View>
                  <Text style={[
                    styles.statusLabel,
                    { color: order?.order_status === 'cancelled' ? '#EF4444' : isActive ? '#111827' : '#9CA3AF' }
                  ]}>
        ...
        <View style={styles.orderCard}>
          ...
          <Text style={styles.infoText}><Text style={styles.label}>Mã đơn: </Text>#{order?.id}</Text>
          <Text style={styles.infoText}><Text style={styles.label}>Địa chỉ: </Text>{order?.address}</Text>
          <Text style={styles.infoText}><Text style={styles.label}>Thanh toán: </Text>{order?.payment_status === 'paid' ? 'Đã thanh toán' : 'Chưa thanh toán'}</Text>
          <Text style={styles.infoText}><Text style={styles.label}>Trạng thái: </Text>{order?.order_status === 'cancelled' ? 'Đã hủy' : STATUS_LABELS[order?.order_status]}</Text>
          <Text style={styles.infoText}><Text style={styles.label}>Tổng tiền: </Text><Text style={styles.totalPrice}>{order?.total_price} đ</Text></Text>
          <Text style={styles.infoText}><Text style={styles.label}>Ngày đặt: </Text>{new Date(order?.created_at).toLocaleString()}</Text>
        </View>
